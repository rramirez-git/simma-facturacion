...
class Manifiestos extends CI_Controller
{
	...
	public function index($idempresa=0,$idsucursal=0)
	{
		...
		$manifiestos=($action == "find" && $idsucursal>0?$this->modmanifiesto->getAll($idsucursal,$filtros):array());
	...
	public function importar($tipo,$idempresa,$idsucursal)
	{
		...
		if($tipo!="excel"&&$tipo!="formiik"&&$tipo!="simmagdl"&&$tipo!="simmadf")
		{
		...
		if(file_exists($archivo))
		{
			...
			foreach($manifs as $k=>$man)
			{
				...
				else
				{
					...
					{
						array_push($warnings,array(3,"El manifesto fue creado para un generador diferente al cual se esta capturando. Generado para {$this->modcliente->getIdentificador()} - {$this->modgenerador->getIdentificador()}, capturando para {$man["cte"]} - {$man["gen"]}. "));
					}
			...
			$head=$this->load->view('html/head',array(),true);
			$menumain=$this->load->view('menu/menumain',array(),true);
			$vista='manifiestos/importar'.($tipo=="simmagdl"||$tipo=="simmadf"?$tipo."_02":"");
			$body=$this->load->view($vista,array(
				"menumain"=>$menumain,
				"idempresa"=>$idempresa,
				"idsucursal"=>$idsucursal,
				"rutas"=>$rutaScheme,
			...
			else if($tipo=="simmagdl")
			{
				switch($celda->getAttribute('columna'))
				{
					...
					case 'M':
						...
						break;
					case 'N':
						$data["res_ext01"]=$celda->nodeValue;
						break;
					case 'O':
						$data["res_ext02"]=$celda->nodeValue;
						break;
					case 'P':
						$data["res_ext03"]=$celda->nodeValue;
						break;
					case 'Q':
						$data["res_ext04"]=$celda->nodeValue;
						break;
					case 'R':
						$data["res_ext05"]=$celda->nodeValue;
						break;
					case 'S':
						$data["res_ext06"]=$celda->nodeValue;
						break;
					case 'T':
						$data["res_ext07"]=$celda->nodeValue;
						break;
					case 'U':
						$data["res_ext08"]=$celda->nodeValue;
						break;
					case 'V':
						$data["res_ext09"]=$celda->nodeValue;
						break;
					case 'W':
						$data["res_ext10"]=$celda->nodeValue;
						break;
					case 'X':
						$data["res_ext11"]=$celda->nodeValue;
						break;
					case 'Y':
						$data["res_ext12"]=$celda->nodeValue;
						break;
					case 'Z':
						$data["res_ext13"]=$celda->nodeValue;
						break;
					case 'AA':
						$data["res_ext14"]=$celda->nodeValue;
						break;
					case 'AB':
						$data["res_ext15"]=$celda->nodeValue;
						break;
				}
			}
			else if($tipo=="simmadf")
			{
				switch($celda->getAttribute('columna'))
				{
					case 'A':
	...
	public function capturarImportacion()
	{
		...
		$r_ext01=$this->input->post('r_ext01');
		$r_ext02=$this->input->post('r_ext02');
		$r_ext03=$this->input->post('r_ext03');
		$r_ext04=$this->input->post('r_ext04');
		$r_ext05=$this->input->post('r_ext05');
		$r_ext06=$this->input->post('r_ext06');
		$r_ext07=$this->input->post('r_ext07');
		$r_ext08=$this->input->post('r_ext08');
		$r_ext09=$this->input->post('r_ext09');
		$r_ext10=$this->input->post('r_ext10');
		$r_ext11=$this->input->post('r_ext11');
		$r_ext12=$this->input->post('r_ext12');
		$r_ext13=$this->input->post('r_ext13');
		$r_ext14=$this->input->post('r_ext14');
		$r_ext15=$this->input->post('r_ext15');
		$ruta=$this->input->post('ruta');
		...
		foreach($settings["sucursales"] as $suc)
		{
			if($suc["id"]==$this->modruta->getIdSucursal())
			{
				...
				foreach($cfgResiduos as $cfgres)
				{
					...
					$this->modrecoleccion->setCantidad(0);
					switch($cfgres["nombre"])
					{
						...
						case 'Medicamento Caduco':
							...
							break;
						default:
							if($suc["id"]=="1")	// SIMMA Polanco
							{
								switch($cfgres["nombre"])
								{
									case 'Basura Industrial':
										$this->modrecoleccion->setCantidad($r_ext01);
										break;
									case 'Fenol y Agua':
										$this->modrecoleccion->setCantidad($r_ext02);
										break;
									case 'Fijador':
										$this->modrecoleccion->setCantidad($r_ext03);
										break;
									case 'Formol':
										$this->modrecoleccion->setCantidad($r_ext04);
										break;
									case 'Placa de Plomo':
										$this->modrecoleccion->setCantidad($r_ext05);
										break;
									case 'Revelador':
										$this->modrecoleccion->setCantidad($r_ext06);
										break;
								}
							}
							else if($suc["id"]=="4")	// SIMMA Giadalajara
							{
								switch($cfgres["nombre"])
								{
									case 'Balastras':
									...
									case 'Filtros de Aire':
										$this->modrecoleccion->setCantidad($r_ext02);
										break;
									case 'Kg. Merma de Proceso':
										$this->modrecoleccion->setCantidad($r_ext03);
										break;
									case 'Porron Alcohol Isopropilico':
										$this->modrecoleccion->setCantidad($r_ext04);
										break;
									case 'Porron de Sanitizante':
										$this->modrecoleccion->setCantidad($r_ext05);
										break;
									case 'Porron de Solventes':
										$this->modrecoleccion->setCantidad($r_ext06);
										break;
									case 'Porron envases Vacios':
										$this->modrecoleccion->setCantidad($r_ext07);
										break;
									case 'Porron Merma de Proceso':
										$this->modrecoleccion->setCantidad($r_ext08);
										break;
									case 'Tambor Basura Industrial':
										$this->modrecoleccion->setCantidad($r_ext09);
										break;
									case 'Tambor de Sanitizante':
										$this->modrecoleccion->setCantidad($r_ext10);
										break;
									case 'Tambor envases con Medicamento':
										$this->modrecoleccion->setCantidad($r_ext11);
										break;
									case 'Tambor Fase Movil':
										$this->modrecoleccion->setCantidad($r_ext12);
										break;
									case 'Tambor Merma de Proceso':
										$this->modrecoleccion->setCantidad($r_ext13);
										break;
									case 'Tambores':
										$this->modrecoleccion->setCantidad($r_ext14);
										break;
									case 'Vidrio Contaminado':
										$this->modrecoleccion->setCantidad($r_ext15);
										break;
								}
							}
							break;
