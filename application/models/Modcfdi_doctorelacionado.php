<?php
class Modcfdi_doctorelacionado extends MY_Model {
	protected $base_table			= "cfdi_doctorelacionado";
	protected $general_view			= "cfdi_doctorelacionado";
	protected $particular_view		= "cfdi_doctorelacionado";
	protected $definition_file		= 'cfdi_doctorelacionado.json';
	protected $order_by				= "serie, folio";
	...
	public function set_field( $field, $value ) {
		...
		if( in_array( $field, array( "monedadr", "tipocambiodr" ) ) ) {
		...
		$this->fields[ $field ] = parent::get_field( 'idcfdi_pago' ) . "___" . parent::get_field( 'iddocumento' );
		return true;
	}
	public function get_field( $field ) {
		if( 'primary_key' == $field ) {
			return parent::get_field( 'idcfdi_pago' ) . "___" . parent::get_field( 'iddocumento' );
		}
	...
	public function get_from_database( $pk_value = "" ) {
		...
		$this->db->where( " concat( idcfdi_pago, '___' , iddocumento ) = '$pk' " );
	...
	public function get_all( $where = "" ) {
		...
		if( 0 < $regs->num_rows() ) {
			...
			foreach( $res as $k => $reg ) {
				$res[ $k ][ "primary_key" ] = $reg[ "idcfdi_pago"] . "___" . $reg[ "iddocumento" ];
			}
		}
		return $res;
	}
