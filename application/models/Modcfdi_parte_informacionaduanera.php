<?php
class Modcfdi_parte_informacionaduanera extends MY_Model {
	protected $base_table			= "cfdi_parte_informacionaduanera";
	protected $general_view			= "cfdi_parte_informacionaduanera";
	protected $particular_view		= "cfdi_parte_informacionaduanera";
	protected $definition_file		= 'cfdi_parte_informacionaduanera.json';
	...
	public function set_field( $field, $value ) {
		...
		$this->fields[ $field ] = parent::get_field( 'idcfdi_parte' ) . "___" . parent::get_field( 'numeropedimento' );
		return true;
	}
	public function get_field( $field ) {
		if( 'primary_key' == $field ) {
			return parent::get_field( 'idcfdi_parte' ) . "___" . parent::get_field( 'numeropedimento' );
		}
	...
	public function get_from_database( $pk_value = "" ) {
		...
		$this->db->where( " concat( idcfdi_parte, '___' , numeropedimento ) = '$pk' " );
	...
	public function get_all( $where = "" ) {
		...
		if( 0 < $regs->num_rows() ) {
			...
			foreach( $res as $k => $reg ) {
				$res[ $k ][ "primary_key" ] = $reg[ "idcfdi_parte"] . "___" . $reg[ "numeropedimento" ];
			}
		}
		return $res;
	}
