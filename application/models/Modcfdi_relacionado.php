<?php
class Modcfdi_relacionado extends MY_Model {
	protected $base_table			= "cfdi_relacionado";
	protected $general_view			= "cfdi_relacionado";
	protected $particular_view		= "cfdi_relacionado";
	protected $definition_file		= 'cfdi_relacionado.json';
	...
	public function set_field( $field, $value ) {
		...
		if( in_array( $field, array( "tiporelacion" ) ) ) {
		...
		$this->fields[ $field ] = parent::get_field( 'idcfdi_comprobante' ) . "___" . parent::get_field( 'idcfdi_comprobante_relacionado' );
		return true;
	}
	public function get_field( $field ) {
		if( 'primary_key' == $field ) {
			return parent::get_field( 'idcfdi_comprobante' ) . "___" . parent::get_field( 'idcfdi_comprobante_relacionado' );
		}
	...
	public function get_from_database( $pk_value = "" ) {
		...
		$this->db->where( " concat( idcfdi_comprobante, '___' , idcfdi_comprobante_relacionado ) = '$pk' " );
	...
	public function get_all( $where = "" ) {
		...
		if( 0 < $regs->num_rows() ) {
			...
			foreach( $res as $k => $reg ) {
				$res[ $k ][ "primary_key" ] = $reg[ "idcfdi_comprobante"] . "___" . $reg[ "idcfdi_comprobante_relacionado" ];
			}
		}
		return $res;
	}
