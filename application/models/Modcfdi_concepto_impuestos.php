<?php
class Modcfdi_concepto_impuestos extends MY_Model {
	protected $base_table			= "cfdi_concepto_impuestos";
	protected $general_view			= "cfdi_concepto_impuestos";
	protected $particular_view		= "cfdi_concepto_impuestos";
	protected $definition_file		= 'cfdi_concepto_impuestos.json';
	...
	public function set_field( $field, $value ) {
		...
		if( in_array( $field, array( "impuesto", "tipofactor" ) ) ) {
		...
		$this->fields[ $field ] = parent::get_field( 'idcfdi_concepto' ) . "___" . parent::get_field( 'tipo' ) . "___" . parent::get_field( 'impuesto' );
		return true;
	}
	public function get_field( $field ) {
		if( 'primary_key' == $field ) {
			return parent::get_field( 'idcfdi_concepto' ) . "___" . parent::get_field( 'tipo' ) . "___" . parent::get_field( 'impuesto' );
		}
	...
	public function get_from_database( $pk_value = "" ) {
		...
		$this->db->where( " concat( idcfdi_concepto, '___' , tipo, '___', impuesto ) = '$pk' " );
	...
	public function get_all( $where = "" ) {
		...
		if( 0 < $regs->num_rows() ) {
			...
			foreach( $res as $k => $reg ) {
				$res[ $k ][ "primary_key" ] = $reg[ "idcfdi_concepto"] . "___" . $reg[ "tipo" ] . "___" . $reg[ "impuesto" ];
			}
		}
		return $res;
	}
