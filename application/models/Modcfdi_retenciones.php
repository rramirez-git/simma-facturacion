<?php
class Modcfdi_retenciones extends MY_Model {
	protected $base_table			= "cfdi_retenciones";
	protected $general_view			= "cfdi_retenciones";
	protected $particular_view		= "cfdi_retenciones";
	protected $definition_file		= 'cfdi_retenciones.json';
	...
	public function set_field( $field, $value ) {
		...
		if( in_array( $field, array( "impuesto" ) ) ) {
		...
		$this->fields[ $field ] = parent::get_field( 'idcfdi_comprobante' ) . "___" . parent::get_field( 'impuesto' );
		return true;
	}
	public function get_field( $field ) {
		if( 'primary_key' == $field ) {
			return parent::get_field( 'idcfdi_comprobante' ) . "___" . parent::get_field( 'impuesto' );
		}
	...
	public function get_from_database( $pk_value = "" ) {
		...
		$this->db->where( " concat( idcfdi_comprobante, '___' , impuesto ) = '$pk' " );
	...
	public function get_all( $where = "" ) {
		...
		if( 0 < $regs->num_rows() ) {
			...
			foreach( $res as $k => $reg ) {
				$res[ $k ][ "primary_key" ] = $reg[ "idcfdi_comprobante"] . "___" . $reg[ "impuesto" ];
			}
		}
		return $res;
	}
