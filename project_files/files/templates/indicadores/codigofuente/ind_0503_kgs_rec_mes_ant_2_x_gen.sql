# Kilogramos Recolectados del dos meses atrás por generador
# Barras
# Cada Residuo
...
__WHR__ AND 
YEAR(man.fecha) = IF(MONTH(CURDATE())=1 OR MONTH(CURDATE())=2,YEAR(CURDATE())-1,YEAR(CURDATE())) AND MONTH(man.fecha) = IF(MONTH(CURDATE())=1,11,IF(MONTH(CURDATE())=2,12,MONTH(CURDATE())-2)) AND cantidad != 0.0
