# Kilogramos Recolectados del tres meses atrás por tipo de residuo
# Barras
...
__WHR__ 
	AND YEAR(man.fecha) = IF(MONTH(CURDATE())=1 OR MONTH(CURDATE())=2 OR MONTH(CURDATE())=3,YEAR(CURDATE())-1,YEAR(CURDATE())) 
	AND MONTH(man.fecha) = IF(MONTH(CURDATE())=1,10,IF(MONTH(CURDATE())=2,11,IF(MONTH(CURDATE())=3,12,MONTH(CURDATE())-3)))
	AND cantidad != 0.0
